...
int main()
{
    ...
    unsigned char *m;
    m = (unsigned char*) malloc(0 * sizeof(char)); //16
    for (i = 0; i < 0; ++i)
    {
        //k[i] = val[i];
        m[i] = val[i % 16];
    }
    unsigned long long mlen = 0; //128
    const unsigned char nsec = '0';
    unsigned char *c;
    c = (unsigned char*) malloc(0 * sizeof(char)); //24
    ...
    printf("Алгоритм работал %f", diffclock(end, start));
    /*for (i = 0; i < 256; ++i)
    {
        printf("%d\n", m[i]);
    }*/
